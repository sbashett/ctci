...
MapValue *get_max_minutes(vector<int> &appointments, int index,
  ...
  if (index ==
      static_cast<int>(appointments.size() - 2)) { // two elements remaining
    if (appointments[index] >= appointments[index + 1])
      mem_table.insert(make_pair(
          index, new MapValue(appointments[index], true, appointments.size())));
    else {
      mem_table.insert(make_pair(
          index, new MapValue(appointments[index + 1], false, index + 1)));
      mem_table.insert(
          make_pair(index + 1, new MapValue(appointments[index + 1], true,
                                            appointments.size())));
    }
    return mem_table[index];
  }
  ...
  if (index ==
      static_cast<int>(appointments.size() - 1)) { // one element remaining
    mem_table.insert(make_pair(
        index, new MapValue(appointments[index], true, appointments.size())));
    return mem_table[index];
  }

  int compVal1 = appointments[index] +
                 get_max_minutes(appointments, index + 2, mem_table)->max_min;
  ...
  if (compVal1 >= compVal2)
    ...
    mem_table.insert(
        make_pair(index, new MapValue(compVal2, false, index + 1)));
