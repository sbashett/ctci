...
bool isKeyExists(T1 key, T2& preprocess_map){
...
void fill_elem_occurences(vector<int>& biggerArr, vector<int>& smallArr, MAPTYPE& preprocess_map){
...
void find_shortest_subseq(MAPTYPE& preprocess_map, int min_diff){
	...
	while(true){		
		...
		temp = heap.top();
		heap.pop();
		preprocess_map[temp->elem_value]->erase(preprocess_map[temp->elem_value]->begin());
