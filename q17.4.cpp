...
int find_missing_number(map<int, bitset<bitset_size>>& numbers_map){
	...
	while(bit_pos < maxBitsCheck){
		...
		cout << "checking bit pos: " << bit_pos << " zeros_count: " << zeros_count << " ones_count: " << ones_count << endl;


		if( (numbers_map.size()+1)%2 == 0 ){ // total numbers count is even num_zeros = num_ones ideally
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char **argv){
	...
	map<int, bitset<bitset_size>> numbers; // change the constant val of bitset size based on usage

	for(int i=0; i <= n; i++){
		numbers[i] = i;
	}

	numbers.erase(49); // change this number to erase a number from list and test the code
