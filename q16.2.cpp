...
public:
  ...
  char letter;
  int freq; // this is the frequency of word made from root char to the current
            // char
  ...
  // returns true if new branch created or false if already exists and freq
  // increases
  bool insert(string);
...
void TrieNode::trimSpaces(string &word) {
  ...
  word.erase(word.begin(), word.begin() + word.find_first_not_of(
                                              whitespaces)); // erase starting
...
bool TrieNode::insert(string word) {
  ...
  transform(word.begin(), word.end(), word.begin(),
            ::tolower); // converting word to lowercase
  trimSpaces(word);
  ...
  for (auto wordIterator : word) {
    ...
    for (childIt = currentNode->children.begin();
         childIt != currentNode->children.end(); childIt++) {
      child = *childIt;
    ...
    if (childIt ==
        currentNode->children
            .end()) { // child not found so start bulding new branches
      ret_flag = true;
