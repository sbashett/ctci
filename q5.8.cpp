...
void draw_intermediate_bytes(BYTE *screen, int start_byte_index, int end_byte_index){
	for(int it=start_byte_index+1; it < end_byte_index; it++){
		screen[it] = 0xFF;
	}
}

void draw_start_offset(BYTE *screen, int start_byte_index, int start_byte_offset){
	BYTE mask = 1;
