...
int getHeight(myspace::BinTreeNode* node){
	...
	if((node->left) != NULL) left_height=getHeight(node->left);
	if((node->right) != NULL) right_height=getHeight(node->right);
...
bool isTreeBalanced(myspace::BinTreeNode* head){
	...
	if((head->left != NULL) || (head->right != NULL)) height=getHeight(head);
	else return true;
...
bool isTreeBST(myspace::BinTreeNode* node, int min_val, int max_val){
	...
	lstree= (node->left==NULL) ? true:isTreeBST(node->left, min_val, node->data);
	rstree= (node->right==NULL) ? true:isTreeBST(node->right, node->data, max_val);
	isBST = lstree & rstree;
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char **argv){
	...
	if(isBalanced) cout << "the binary tree is BALANCED\n";
	else cout << "the binary tree is NOT BALANCED\n";

	bool isBST = isTreeBST(head, INT_MIN, INT_MAX);

	if(isBST) cout << "the binary tree is a BINARY SEARCH TREE\n";
