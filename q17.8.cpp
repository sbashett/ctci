...
void longest_subseq_helper(int currentIndex, vector<int>& array, map<int, vector<int>>& longest_subseq_map){
	
	if(currentIndex == 0){
		longest_subseq_map.insert(make_pair(currentIndex, vector<int>{array[currentIndex]}) );
		return;
	}
