...
bool doesCombExist_helper(string, string, vector<string>&, map<string, bool>&);

bool checkWord(string word, vector<string>& list_words, map<string, bool>& mem_map){	
...
bool doesCombExist_helper(string lword, string rword, vector<string>& list_words, map<string, bool>& mem_map){
...
bool doesCombExist(string word, vector<string>& list_words, map<string, bool>& mem_map){
	bool combExist;
	for(int i = 1; i < static_cast<int>(word.size()); i++){
		combExist = doesCombExist_helper(word.substr(0, i), word.substr(i), list_words, mem_map);
		// cout << word.substr(0, i+1) << ", " << word.substr(i+1, word.size()-1) << ": " << combExist << endl;
		if(combExist)
			return true;
	}
	return false;
...
int main(void){
	vector<string> list_words{"cat", "banana", "dog", "nana", "walk", "walker", "dogwalker"};
	sort(list_words.begin(), list_words.end(), sort_func ); // sort in decreasing order of length of strings
	for(auto& word : list_words)
		cout << word << endl;
