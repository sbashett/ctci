...
void fill_table(string word, int pos, TABLEMAP& lut){
	transform(word.begin(), word.end(), word.begin(), ::tolower); // converting everything to lower case
...
string trim_and_transform(string word){
	...
	word = ( (start == end) && ((int)word[start] == 32) ) ? "" : word.substr(start, end-start+1);
...
void find_shortest_distance(string word1, string word2, TABLEMAP& lut){
	...
	cout << "shortest distance between " << word1 << " and " << word2 << " is " << shortest_dist << endl;
