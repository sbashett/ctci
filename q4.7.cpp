...
void Graph::create_graph(vector<vector<int>> &edges, vector<int> &all_nodes) {
  ...
  for (auto &edge : edges) {
    ...
    find_iterator_parent->second->children.push_back(
        find_iterator_child->second);
    find_iterator_child->second->parents.push_back(
        find_iterator_parent->second);
  }
...
bool isExist(Graphspace::GraphMap map_nodes, int node_val) {
  ...
  if (find_iter == map_nodes.end())
    return false;
  else
    return true;
}

void remove_nodes_vector(vector<Graphspace::GraphNode *> &vec,
                         vector<Graphspace::GraphNode *> &nodes) {
  for (auto &n : nodes) {
...
bool checkAllDependencies(vector<Graphspace::GraphNode *> &parents,
                          const vector<int> &final_sequence) {
  // cout << "parents: \n";
...
bool get_final_sequence(vector<int> &final_sequence,
  ...
  while (final_sequence.size() != all_nodes.size()) {
    ...
    if (final_sequence.size() == all_nodes.size())
      return true;
    if (!(frstFlag) && !(final_sequence.size() > prev_final_seq_size)) {
      // cout << prev_final_seq_size << ',' << final_sequence.size() << '\n';
      return false;
    } else
      prev_final_seq_size = final_sequence.size();

    frstFlag = false;

    for (auto &node : all_nodes) {
      if (isExist(final_sequence, node))
        continue;
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char **argv) {
  ...
  if (!isSeqValid)
    cout << "could not find a sequence probably there is a cycle in dependency "
            "graph\n";
  else {
    cout << "sequence is:\n";
    for (auto &node : final_sequence)
      cout << node << ", ";
  }
  cout << '\n';
