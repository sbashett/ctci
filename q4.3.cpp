...
void generate_ll_depths(myspace::BinTreeNode *head, map<int, myspace::SingleLL*>& linkedLists){
	...
	while(!(process_queue.empty())){
		...
		if(linkedLists.find(currentNode->level) == linkedLists.end()){
			linkedLists[currentNode->level] = new myspace::SingleLL(currentNode->data);
		}
		else linkedLists[currentNode->level]->appendToTail(currentNode->data);
