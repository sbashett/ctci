...
void replace_row_with_zeros(int *matrix, int row, int N){
	for(int cols=0; cols<N; cols++) *(matrix + row*N + cols)=0;
}

void replace_col_with_zeros(int *matrix, int col, int M, int N){
	for(int rows=0; rows<M; rows++) *(matrix + rows*N + col)=0;
}
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char **argv){
	...
	vector<int> vector_rows_zero, vector_cols_zero, vector_rem_rows, vector_rem_cols;

	for(it=0; it<M; it++) list_rows.push_back(it);
	for(it=0; it<N; it++) list_cols.push_back(it);
	
	START_LOOP:
		if(vector_rows_zero.size() > 0) 
			for(int rows : vector_rows_zero) replace_row_with_zeros(&matrix[0][0], rows, N);
		if(vector_cols_zero.size() > 0)
			for(int cols : vector_cols_zero) replace_col_with_zeros(&matrix[0][0], cols, M, N);
		for(int rows : vector_rem_rows) list_rows.remove(rows);
		for(int cols : vector_rem_cols) list_cols.remove(cols);
