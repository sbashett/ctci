...
typedef int INDICES
typedef map<INDICES, vector<LLNode*>> MYMAP;
...
public:
	...
	void print_graph();
	void resetNodeStates(); //includes resetting the state variable and the dist variable
	vector<GraphNode*> shortestPath(int); //breadth first search to find path to destination
...
GraphNode* Graph::getNodePointer(int val){
	return this->map_nodes[val];
}
